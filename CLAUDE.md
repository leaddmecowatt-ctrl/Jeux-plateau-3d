...
| `scene/relooking.js` | **inactif** (`RELOOKING_PION = false` dans `pion.js`, l'animateur préfère le personnage d'origine) : le personnage façon figurine : chapeau de paille modélisé (posé sur l'os de la tête, casquette masquée), visage redessiné au canvas (grands yeux, sourire, cicatrice, clignement en trait), gilet / ceinture / short retravaillés par couleur. Propre à ce modèle. |
