...
/* La règle d’argent — marge, prix des lots, recettes, construction de la
...
/* La règle de rentabilité (marge, plafond, prix des lots, recettes) est
   dans regles/argent.js — importée en haut de ce fichier. Elle ne
   dépend ni du plateau ni du navigateur, et se vérifie avec `node
   --test tests/`. Ne rien régler ici : tout se règle là-bas. */

/* Recalibrage : le plan (RECAL) est dans regles/argent.js. Ici ne
   restent que la clé de mémorisation et l'application, qui touchent au
   navigateur. */
...
/* Plafond de reversement cumulé pour la cagnotte encaissée. La règle
   est dans regles/argent.js ; ici on ne fait que lui passer l'état du
   recalibrage en cours. */
function ceilingFor(mise){ return plafondPour(mise, recal); }
...
/* Encaisse le lot réellement finançable pour la catégorie demandée et
   met à jour le total reversé. La DÉCISION est dans regles/argent.js
   (tierFor, pure et testée) ; ici il ne reste que la tenue des
   compteurs. */
function fundedCategory(catKey){
  const tier = tierFor(catKey, totalMise, totalPaid, recal);
...
/* La file des lots décidés à l’avance (comment elle est construite et
   pourquoi) est expliquée dans regles/argent.js. Ici : sa mémorisation
   dans le navigateur, et rien d’autre. */
const OUTCOME_BATCH_KEY = 'pika_outcome_batch';
// Chances par partie de chaque lot, pour la plaque centrale du plateau
// et la légende télé. Le calcul est dans regles/argent.js ; ici on ne
// fait qu’en peindre le résultat.
{
  LOT_ODDS_PCT = chancesParLot();
  renderTvLegend();
...
/* Garantie finale en direct : la file est construite pour tenir le
   plafond à 9 € par partie, mais la réalité peut diverger (parties
   recommencées sans valider, validation anticipée, ancienne file…).
   La règle est dans regles/argent.js. */
function outcomeCovered(cat){ return estCouvert(cat, totalMise, totalPaid, recal); }
